...
/**
  ...
  * @remark		
  */
unsigned short int  MyQueue_Count(const MyQueue_Typedef *queue)
{
	if(queue==NULL)
		return 0;
...
/**
  ...
  * @remark		
  */
unsigned short int MyQueue_LeftCount(const MyQueue_Typedef *queue)
{
	if(queue==NULL)
		return 0;
...
bool MyQueue_IsFull(const MyQueue_Typedef *queue)
{
	...
	return MyQueue_Count(queue)  == ((queue->len)-1);
}
...
bool MyQueue_Put(MyQueue_Typedef *queue,const void *buf,unsigned short int num)
{
	...
	if( num > MyQueue_LeftCount(queue))
		return false;
	
	if(queue->w_lock == true)
	{
		return false;
	}
...
bool MyQueue_Get(MyQueue_Typedef *queue,void *buf,unsigned short int num)
{
	...
	if(num > MyQueue_Count(queue))
		return false;
	
	if(queue->r_lock == true)
	{
		return false;
	}
...
bool MyQueue_Peek(const MyQueue_Typedef *queue,void *buf,unsigned short int num,unsigned short int offset)
{
	...
	if((offset+num) > (MyQueue_Count(queue)))
		return false;
